...
const startServer = () => {
    ...
    channels.forEach((channel, index) => {
        ...
        app.get('/', (req, res) => {
            res.send(`server is listinign - ${channel.name} --> ${channel.multicastAddress} ---> http://${channel.ipaddress}:${PORT}/stream.ts`)
        })


        http.createServer(app).listen(PORT, channel.ipaddress, () => {
            console.log("server is listining - ", channel.name, "--->", `http://${channel.ipaddress}:${PORT}`)
        })
    })
}
